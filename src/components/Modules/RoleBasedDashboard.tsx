...
export const RoleBasedDashboard: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      {/* 1-Click Role Switcher Ribbon */}
      <div className="bg-slate-900 text-white rounded-2xl p-4.5 border border-slate-800 shadow-md space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800 pb-2.5">
          <div className="flex items-center gap-2">
            <span className="text-amber-400 font-bold text-xs uppercase tracking-wider flex items-center gap-1.5">
              ...
              Role-Specific Command Perspective
            </span>
            <span className="text-slate-500 text-xs">• Select your role below to experience live dashboards & actions:</span>
          </div>
          <span className="text-[11px] text-emerald-400 font-semibold">
            Active: {currentRoleMeta.label}
          </span>
        </div>
          ...
          {availableRoles.map((r) => {
            ...
            return (
              <button
                ...
                className={`px-2.5 py-2 rounded-xl text-xs font-bold transition-all text-left flex flex-col justify-between gap-1 border ${
                  isSelected
                    ? "bg-blue-600 text-white border-blue-400 shadow-sm scale-102"
              ...
              >
                ...
                <span className="text-[11px] leading-tight truncate">{r.id.replace("Officer_", "")}</span>
              </button>
            );
          })}
        </div>
          ...
          {assignedProjects.map((p) => (
            ...
            >
              ...
              <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs">
                <button
                  onClick={() => handleStartEdit(p)}
                  className="px-3 py-1 bg-[#0f2b48] hover:bg-[#1a3d60] text-white font-bold rounded text-[11px] flex items-center gap-1"
                >
